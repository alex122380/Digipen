...
void UpdateScene();
void RenderScene();
void RenderToTexture(int target);
void RenderToScreen();
...
int main()
{
  ...
  while (!pWindowSystem->WindowShouldClose())
  {
    ...
    //RenderScene();
    pWindowSystem->currentCam = pWindowSystem->cam;
    pRenderer->SetCamera(*pWindowSystem->currentCam);
    RenderToTexture(0);
    pWindowSystem->currentCam = pWindowSystem->cam;
    pRenderer->SetCamera(*pWindowSystem->currentCam);
    RenderToScreen();
...
void UpdateScene()
{
  ...
  // Set Square transformation.
  tCubeFront = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -5.0f));
  tCubeFront = glm::rotate(tCubeFront, glm::radians(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
  tCubeFront = glm::scale(tCubeFront, glm::vec3(10.0f, 10.0f, 10.0f));
  tCubeRight = glm::translate(glm::mat4(1.0f), glm::vec3(5.0f, 0.0f, 0.0f));
  tCubeRight = glm::rotate(tCubeRight, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
  tCubeRight = glm::scale(tCubeRight, glm::vec3(10.0f, 10.0f, 10.0f));
  tCubeLeft = glm::translate(glm::mat4(1.0f), glm::vec3(-5.0f, 0.0f, 0.0f));
  tCubeLeft = glm::rotate(tCubeLeft, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
  tCubeLeft = glm::scale(tCubeLeft, glm::vec3(10.0f, 10.0f, 10.0f));
  tCubeBottom = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -5.0f, 0.0f));
  tCubeBottom = glm::rotate(tCubeBottom, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
  tCubeBottom = glm::scale(tCubeBottom, glm::vec3(10.0f, 10.0f, 10.0f));
  tCubeTop = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 5.0f, 0.0f));
  tCubeTop = glm::rotate(tCubeTop, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
  tCubeTop = glm::scale(tCubeTop, glm::vec3(10.0f, 10.0f, 10.0f));
  tCubeBack = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 5.0f));
  tCubeBack = glm::rotate(tCubeBack, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
  tCubeBack = glm::scale(tCubeBack, glm::vec3(10.0f, 10.0f, 10.0f));
...
void RenderScene()
{
  ...
  // Render to texture.
  pRenderer->SetFrameBuffer(Renderer::FRAME_BUFFER::TO_TEXTURE);
  glClearColor(0.5f, 0.5f, 0.5f, 1.0f);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  //pRenderer->SetFrameBuffer(Renderer::FRAME_BUFFER::DEFAULT);
  ...
  pRenderer->SetFrameBuffer(Renderer::FRAME_BUFFER::DEFAULT);
  pRenderer->SetShader(Shader::SHADER_TYPE::WITH_TEXTURE);
  pRenderer->AddMesh(pMeshDataSquare);
  pRenderer->AddBatch();
  glBindTexture(GL_TEXTURE_2D, pRenderer->GetTextureId());
  pRenderer->SetModelMatrix(tCubeFront);
  pRenderer->SetModelViewMatrix(tCubeFront, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeRight);
  pRenderer->SetModelViewMatrix(tCubeRight, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeLeft);
  pRenderer->SetModelViewMatrix(tCubeLeft, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeBottom);
  pRenderer->SetModelViewMatrix(tCubeBottom, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeTop);
  pRenderer->SetModelViewMatrix(tCubeTop, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeBack);
  pRenderer->SetModelViewMatrix(tCubeBack, *pWindowSystem->cam);
  pRenderer->Render();
  pRenderer->ClearMesh();
#endif
}
...
void RenderToTexture(int target)
{
  ...
  pRenderer->AddBatch();
  pRenderer->SetModelMatrix(tObject);
  pRenderer->SetModelViewMatrix(tObject, *pWindowSystem->currentCam);
  //glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
  //glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  //glEnable(GL_DEPTH_TEST);
  glViewport(0, 0, 1280, 720);
  ...
  {
    pRenderer->SetModelMatrix(tSphere[i]);
    pRenderer->SetModelViewMatrix(tSphere[i], *pWindowSystem->currentCam);
    //glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
    //glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    //glEnable(GL_DEPTH_TEST);
    glViewport(0, 0, 1280, 720);
  ...
  pRenderer->AddBatch();
  pRenderer->SetModelMatrix(tOrbit);
  pRenderer->SetModelViewMatrix(tOrbit, *pWindowSystem->currentCam);
  //glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
  //glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  //glEnable(GL_DEPTH_TEST);
  glViewport(0, 0, 1280, 720);
  ...
  pRenderer->AddBatch();
  pRenderer->SetModelMatrix(tFloor);
  pRenderer->SetModelViewMatrix(tFloor, *pWindowSystem->currentCam);
  glViewport(0, 0, 1280, 720);
  glCullFace(GL_FRONT);
  glFrontFace(GL_CCW);
  pRenderer->Render();
...
void RenderToScreen()
{
  ...
  glBindTexture(GL_TEXTURE_2D, pRenderer->GetTextureId());
  pRenderer->SetModelMatrix(tCubeFront);
  //pRenderer->SetModelViewMatrix(tCubeFront, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeRight);
  //pRenderer->SetModelViewMatrix(tCubeRight, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeLeft);
  //pRenderer->SetModelViewMatrix(tCubeLeft, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeBottom);
  //pRenderer->SetModelViewMatrix(tCubeBottom, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeTop);
  //pRenderer->SetModelViewMatrix(tCubeTop, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->SetModelMatrix(tCubeBack);
  //pRenderer->SetModelViewMatrix(tCubeBack, *pWindowSystem->currentCam);
  pRenderer->Render();
  pRenderer->ClearMesh();
}
...
MeshData* CreateSquare()
{
  ...
  std::vector<MeshData::Vertex> vertices;
  // position.
  glm::vec3 p0 = glm::vec3( 0.5f,  0.5f,  0.0f); // top right
  glm::vec3 p1 = glm::vec3( 0.5f, -0.5f,  0.0f); // bottom right
  glm::vec3 p2 = glm::vec3(-0.5f, -0.5f,  0.0f); // bottom left
  glm::vec3 p3 = glm::vec3(-0.5f,  0.5f,  0.0f); // top left
  // normal.
  glm::vec3 n0 = glm::vec3( 0.0f,  0.0f,  1.0f); // top right
  glm::vec3 n1 = glm::vec3( 0.0f,  0.0f,  1.0f); // bottom right
  glm::vec3 n2 = glm::vec3( 0.0f,  0.0f,  1.0f); // bottom left
  glm::vec3 n3 = glm::vec3( 0.0f,  0.0f,  1.0f); // top left
  // texture coordinate.
  //glm::vec2 t0 = glm::vec2(1.0f, 0.0f); // top right
  //glm::vec2 t1 = glm::vec2(1.0f, 1.0f); // bottom right
  //glm::vec2 t2 = glm::vec2(0.0f, 1.0f); // bottom left
  //glm::vec2 t3 = glm::vec2(0.0f, 0.0f); // top left
  glm::vec2 t0 = glm::vec2(1.0f, 1.0f); // top right
  glm::vec2 t1 = glm::vec2(1.0f, 0.0f); // bottom right
  glm::vec2 t2 = glm::vec2(0.0f, 0.0f); // bottom left
  glm::vec2 t3 = glm::vec2(0.0f, 1.0f); // top left
  // color.
  glm::vec4 c0 = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f); // top right
  glm::vec4 c1 = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f); // bottom right
  glm::vec4 c2 = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f); // bottom left
  glm::vec4 c3 = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f); // top left
